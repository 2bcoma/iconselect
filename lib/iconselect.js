...
export default class IconSelect {
	constructor(element, parameters = {}) {
		...
		this._params = { ...defaultParams, ...parameters }
...
class View {
	...
	createUI(parameters) {
		...
		this.selectedIconImgElement.style.borderRadius =
			parameters.selectedIconRadius + 'px'
		...
		componentIconElement.style.bottom =
			-1 + parameters.selectedBoxPadding + 'px'
	...
	createIcon(iconFilePath, iconValue, index, parameters) {
		...
		iconImgElement.style.borderRadius = parameters.iconsRadius + 'px'
